...
def analisar():
    ...
    # Sobe 3 níveis: utils -> excel -> app -> raiz
    raiz_projeto = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
    caminho_carros = os.path.join(raiz_projeto, "data", "carros.xlsx")
    
    print(f"📂 Procurando arquivo em: {caminho_carros}")
    
    if not os.path.exists(caminho_carros):
        print(f"❌ Arquivo não encontrado: {caminho_carros}")
        return None, 0
    
    df = pd.read_excel(caminho_carros)
    ...
    arquivo = os.path.join(raiz_projeto, "data", "oportunidades.xlsx")
